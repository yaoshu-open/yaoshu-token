...
const APP_TEMPLATES: Array<
  Omit<AppRanking, 'rank' | 'monthly_tokens' | 'growthPct' | 'initial'>
> = [
  {
    name: 'Cline',
...
export function buildGroupPerformance(model: PricingModel): GroupPerformance[] {
  const groups = (model.enableGroup ?? []).filter((g) => g && g !== 'auto')
  const targets = groups.length > 0 ? groups : ['default']
  const profile = PROFILE_BY_NAME(model.modelName)
  const spec = PROFILE_SPECS[profile]
  const baseSeed = hashStringToSeed(model.modelName)
...
export function buildLatencyTimeSeries(
  ...
  const baseSeed = hashStringToSeed(`${model.modelName}:lat`)
...
export function buildUptimeSeries(
  ...
  const baseUptime = target
    ...
    : performances.reduce((s, p) => s + p.uptime_30d_pct, 0) /
      performances.length
  const baseSeed = hashStringToSeed(`${model.modelName}:up:${group ?? '_all'}`)
...
export function buildAppRankings(
  ...
  const baseSeed = hashStringToSeed(`${model.modelName}:apps`)
  const rand = seededRandom(baseSeed)
...
function apiCategoryOf(model: PricingModel): ApiCategory {
  ...
  if (profile === 'image') {
    return /sora|veo|kling|pika|video|wan-|hunyuanvideo/i.test(model.modelName)
      ? 'video'
      : 'image'
  }
  return 'chat'
...
export function buildRateLimits(model: PricingModel): RateLimit[] {
  ...
  const targets = groups.length > 0 ? groups : ['default']
  const cat = apiCategoryOf(model)
  const baseSeed = hashStringToSeed(`${model.modelName}:rl`)
