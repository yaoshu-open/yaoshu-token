import type { Modality, ModelCapability, PricingModel } from '@/api/pricing/types'
import { hashStringToSeed, seededRandom, randomInRange } from './seed'

const TEXT_INPUT_ENDPOINTS = new Set([
  'openai', 'openai-response', 'anthropic', 'gemini', 'embeddings', 'jina-rerank'
])
...
const REASONING_NAME_PATTERNS = [
  /^o[1-4](?:[-:_].+)?$/i, /reasoning/i, /thinking/i, /qwq/i,
  /deepseek-r\d/i, /grok.*-(?:thinking|reasoning)/i
]
...
const KNOWLEDGE_CUTOFFS = [
  '2023-04', '2023-10', '2023-12', '2024-04', '2024-06', '2024-08',
  '2024-10', '2024-12', '2025-02', '2025-04', '2025-08'
]
const PARAM_BUCKETS = ['1.5B', '3B', '7B', '8B', '14B', '32B', '70B', '120B', '405B']
const CONTEXT_BUCKETS = [8_192, 16_384, 32_768, 65_536, 128_000, 200_000, 1_000_000]
const MAX_OUTPUT_BUCKETS = [2_048, 4_096, 8_192, 16_384, 32_768, 65_536]

const TAG_TO_CAPABILITY: Record<string, ModelCapability> = {
  vision: 'vision', multimodal: 'vision', reasoning: 'reasoning', thinking: 'reasoning',
  tools: 'tools', function: 'function_calling', 'function-calling': 'function_calling',
  streaming: 'streaming', json: 'json_mode', structured: 'structured_output',
  search: 'web_search', code: 'code_interpreter', embedding: 'embeddings'
}
...
function parseModelTags(tagsString?: string): string[] {
  ...
  return tagsString.split(/[,;|\s]+/).map((t) => t.trim().toLowerCase()).filter(Boolean)
}
...
function inferInputModalities(
  model: PricingModel, tags: string[], endpoints: string[], name: string
): Modality[] {
  const set = new Set<Modality>()
  if (endpoints.length === 0 || endpoints.some((e) => TEXT_INPUT_ENDPOINTS.has(e))) set.add('text')
  if (model.imageRatio != null || nameMatches(name, VISION_NAME_PATTERNS)) set.add('image')
  if (model.audioRatio != null || nameMatches(name, AUDIO_NAME_PATTERNS)) set.add('audio')
...
function inferOutputModalities(
  model: PricingModel, endpoints: string[], name: string
): Modality[] {
  const set = new Set<Modality>()
  ...
  if (model.audioCompletionRatio != null || /tts|voice|audio-out/i.test(name)) set.add('audio')
  if (set.size === 0) set.add('text')
  return ordered(set)
}
...
function inferCapabilities(
  ...
  if (!endpoints.includes('image-generation') && !endpoints.includes('embeddings') && !endpoints.includes('jina-rerank')) {
  ...
  if (model.cacheRatio != null) set.add('caching')
...
function inferContextAndOutputs(
  name: string, rand: () => number, endpoints: string[]
  ...
  if (lower.includes('1m') || lower.includes('-long')) return { context: 1_000_000, maxOutput: 65_536 }
  if (/claude.*(?:4|opus|sonnet)/.test(lower)) return { context: 1_000_000, maxOutput: 65_536 }
  if (lower.includes('200k') || lower.includes('claude-3') || lower.includes('claude-4')) return { context: 200_000, maxOutput: 16_384 }
  if (lower.includes('128k') || /gpt-4o|gpt-4\.1|gpt-5|o1|o3|o4/.test(lower)) return { context: 128_000, maxOutput: 16_384 }
  if (/gemini.*-2|gemini.*pro|gemini.*flash/.test(lower)) return { context: 1_000_000, maxOutput: 8_192 }
  if (/gpt-3\.5|claude-2/.test(lower)) return { context: 16_384, maxOutput: 4_096 }
...
function inferReleaseAndCutoff(rand: () => number): { release: string; cutoff: string } {
...
/** 推断/模拟模型元数据。优先使用 model.* 上的显式字段，否则回退到推断 + 确定性种子 */
export function inferModelMetadata(model: PricingModel): ModelMetadata {
  ...
  const inputs = model.inputModalities ?? inferInputModalities(model, tags, endpoints, name)
  const outputs = model.outputModalities ?? inferOutputModalities(model, endpoints, name)
  const capabilities = model.capabilities ?? inferCapabilities(model, tags, endpoints, name, outputs, inputs)
  ...
  return {
    ...
    maxOutputTokens: model.maxOutputTokens ?? fallback.maxOutput,
    knowledgeCutoff: model.knowledgeCutoff ?? cutoffAndRelease.cutoff,
    releaseDate: model.releaseDate ?? cutoffAndRelease.release,
    parameterCount: model.parameterCount ?? pickFromBuckets(PARAM_BUCKETS, rand),
...
const TOKEN_FORMAT = new Intl.NumberFormat(undefined, { maximumFractionDigits: 1 })

/** 格式化 token 数量：128_000 → "128K"，1_000_000 → "1M" */
...
/** 格式化 YYYY-MM（或 YYYY-MM-DD）为 "Mon YYYY" */
...
// ---------------------------------------------------------------------------
// Provider / vendor / tokenizer / license 推断
// ---------------------------------------------------------------------------

export type ModelVendor =
  | 'openai' | 'anthropic' | 'google' | 'meta' | 'mistral' | 'qwen'
  | 'deepseek' | 'xai' | 'cohere' | 'baidu' | 'zhipu' | 'moonshot'
  | 'minimax' | 'tencent' | 'bytedance' | 'midjourney' | 'stability' | 'unknown'
...
const VENDOR_LABELS: Record<ModelVendor, string> = {
  ...
  mistral: 'Mistral AI', qwen: 'Alibaba (Qwen)', deepseek: 'DeepSeek', xai: 'xAI',
  cohere: 'Cohere', baidu: 'Baidu', zhipu: 'Zhipu AI', moonshot: 'Moonshot AI',
  minimax: 'MiniMax', tencent: 'Tencent', bytedance: 'ByteDance',
  midjourney: 'Midjourney', stability: 'Stability AI', unknown: 'Unknown'
}

function detectVendor(name: string): ModelVendor {
  const n = name.toLowerCase()
  if (/^gpt|^o[1-4]|davinci|babbage|whisper|tts|dall.?e|sora|^omni/.test(n)) return 'openai'
...
const TOKENIZER_BY_VENDOR: Partial<Record<ModelVendor, string>> = {
  openai: 'o200k_base', anthropic: 'Anthropic Claude tokenizer',
  google: 'SentencePiece (Gemini)', meta: 'Llama 3 tokenizer',
  mistral: 'Mistral tokenizer (BPE)', qwen: 'Qwen tokenizer (tiktoken-compat)',
  deepseek: 'DeepSeek tokenizer (BPE)', xai: 'Grok tokenizer (BPE)',
  cohere: 'Cohere tokenizer', baidu: 'Ernie tokenizer', zhipu: 'GLM tokenizer',
  moonshot: 'Kimi tokenizer', minimax: 'ABAB tokenizer', tencent: 'Hunyuan tokenizer',
  bytedance: 'Doubao tokenizer'
}

function inferTokenizer(model: PricingModel, vendor: ModelVendor): { tokenizer: string; note?: string } {
...
const LICENSE_BY_VENDOR: Record<ModelVendor, { license: string; kind: ApiInfo['license_kind'] }> = {
...
/** 构建 vendor / tokenizer / license / privacy 元数据 */
export function inferApiInfo(model: PricingModel): ApiInfo {
  const vendor = detectVendor(model.modelName || '')
  const tk = inferTokenizer(model, vendor)
  const license = LICENSE_BY_VENDOR[vendor]
  const rand = seededRandom(hashStringToSeed(`${model.modelName}:api`))
  const retention = vendor === 'openai' ? 30 : Math.round(randomInRange(rand, 0, 90))
  return {
    vendor,
