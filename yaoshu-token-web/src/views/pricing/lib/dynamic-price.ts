...
import { TOKEN_UNIT_DIVISORS } from '@/api/pricing/constants'
...
export function isDynamicPricingModel(model: PricingModel): boolean {
  return model.billingMode === 'tiered_expr' && Boolean(model.billingExpr)
}

export function getDynamicDisplayGroupRatio(model: PricingModel): number {
  const groups = Array.isArray(model.enableGroup) ? model.enableGroup : []
  const ratios = model.groupRatio || {}
...
export function formatDynamicUnitPrice(
  ...
  const priceUSD =
    (valuePerMillionTokens * groupRatio) / TOKEN_UNIT_DIVISORS[options.tokenUnit]
...
export function getDynamicPricingTiers(model: PricingModel): ParsedTier[] {
  ...
  const { billingExpr } = splitBillingExprAndRequestRules(model.billingExpr || '')
...
export function hasDynamicRequestRules(model: PricingModel): boolean {
  ...
  const { requestRuleExpr } = splitBillingExprAndRequestRules(model.billingExpr || '')
...
export function getDynamicPricingSummary(
  ...
  const rawExpression = model.billingExpr || ''
  return {
    tiers,
    tier,
    ...
    rawExpression,
    entries,
    primaryEntries: entries.filter((e) => PRIMARY_DYNAMIC_FIELDS.has(e.field)),
    secondaryEntries: entries.filter((e) => !PRIMARY_DYNAMIC_FIELDS.has(e.field))
  }
}
